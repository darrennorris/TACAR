...
Split into three parts: 
 ...
 - Projections    
 This section includes code and process to calculate deterministic 
 and stochastic population projections using stage-based 
 Matrix Population Models.
 
 - Scenario integration    
 This section integrates the population projections across spatially 
 explicit model scenarios.

 - Results    
 Develop graphics to clearly communicate the results obtained.
...
functions in the package Rage (https://jonesor.github.io/Rage/).
...
### Section objective
Effectively communicate the results obtained.
...
long names if the basin-country combinations were on the x-axis.
...
The shaded shapes in the above figure show predicted population changes 
for each basin-country combination. 
...
shaded area is the interquartile range (IQR - 25% and 75% quantiles). 
As the basin-country combinations cover different sized areas, 
estimates of overall mean and IQR are obtained using  
calculations weighted by the river length.
A population change of "-0.5" is 50% loss. 

Horizontal black crossbars show the median and interquartile range of 
projection scenarios within each basin-country combination. These scenarios 
represent the impact of threats (e.g. hunting) across three generations.
The impact of threats ranges from most severe ("pessimistic"), 
Business-as-usual ("BAU"), to least severe ("optimistic").
Generation length was estimated from stage-based matrix population models, 
and results include scenario results from mean (41 year), and the 
interquartile range (35 and 45 year, 25% and 75% quantile values respectively).
Generation length follows the IUCN definition: 
