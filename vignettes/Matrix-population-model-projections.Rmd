...
resource_files:
  - 'man/figures/fig_basin_country_summary.png'
---
...
# Make factors
df$model_namef <- factor(df$model_name)
levels(df$model_namef)
df$model_namef <- relevel(df$model_namef, ref = "modelkey_pessimistic")
df$modelidf <- factor(df$modelid)
...
# Export for future use
png(file = "inst/other/fig_basin_country_summary.png", bg = "transparent", 
    type = c("cairo"), 
    width = 9, height = 11, units = "in", res=600)
fig_basin_country
invisible(dev.off())
png(file = "man/figures/fig_basin_country_summary.png", bg = "transparent", 
    type = c("cairo"), 
    width = 9, height = 11, units = "in", res=600)
fig_basin_country
invisible(dev.off())
# below does not work as needs function #> ! object 'iqr' of mode 'function' was not found
saveRDS(fig_basin_country, "inst/other/fig_basin_country_summary.rds")
```

Check plot.
```{r fig-check-plot, echo=FALSE}
# Define function to calculate IQR at given quantiles
iqr = function(z, lower = 0.25, upper = 0.75) {
  data.frame(
    y = median(z),
...
fig_basin_country <- readRDS(system.file("other/fig_basin_country_summary.rds", 
                                         package = "TACAR"))
fig_basin_country
#knitr::include_graphics('man/figures/fig_basin_country_summary.png') 
```
