...
geom_col_name <- attr(ffr_1a4_poun_3395,"sf_column")
...
ffr_1a4_poun_3395 <- st_crop(ffr, basin_bbox) |> st_transform(3395)
ffr_1a4_poun_3395_basins <- sf::st_intersection(ffr_1a4_poun_3395, basins_clean_3395)
...
# Then Amazon by subbasin
# 1) Load Free-flowing rivers made previously.
infile <- "C:\\Users\\user\\Documents\\Articles\\2024_Norris_Greenstatus\\TACAR\\inst\\vector\\poun_rivers.gpkg"
ffr_1a4_poun_3395 <- st_read(infile, layer = "ffr_1a4_poun_3395")

# overall length 160,208. Norris et al. 2019 was 215,975. 165 Access, 50 not.
# 76% rivers accessible. 57.2% Accessible and unprotected.
lenth_tot <- as.numeric(sum(st_length(ffr_1a4_poun_3395))) / 1000
#Number of reaches by basin
 table(ffr_1a4_poun_3395$BASIN_NAME)
# Amazon    Coastal North  Coastal South   Orinoco 
# 51871          3728           945          8476 
 basiin_river_lengths <- ffr_1a4_poun_3395 |> 
   mutate(reach_length_km = as.numeric(units::set_units(sf::st_length(ffr_1a4_poun_3395), km))) |> 
   data.frame() |>
   group_by(BASIN_NAME) |> 
   summarise(river_km = sum(reach_length_km)) |> 
   ungroup()
basiin_river_lengths
#  BASIN_NAME    river_km
#1 Amazon         128771.
...
df_ffr_1a4_poun_3395_south <- ffr_1a4_poun_3395 |> 
  dplyr::filter(BASIN_NAME == "Coastal South") |> data.frame()
df_ffr_1a4_poun_3395_north <- ffr_1a4_poun_3395 |> 
  dplyr::filter(BASIN_NAME == "Coastal North") |> data.frame()
df_ffr_1a4_poun_3395_orinoco <- ffr_1a4_poun_3395 |> 
  dplyr::filter(BASIN_NAME == "Orinoco") |> data.frame()
df_ffr_1a4_poun_3395_amazon <- ffr_1a4_poun_3395 |> 
...
dfpoints_south <- ddply(df_ffr_1a4_poun_3395_south, .(REACH_ID), 
                        .fun = river_points)
...
pe <- st_as_sf(extract(uas_3395, vect(ffr_1a4_poun_points_3395), bind = TRUE))
pe |> data.frame() |> select(Indigenous, Dist..km.) |> summary()
# check NAs. All 20 where rivers join oceans north east.
...
ffr_1a4_poun_points_3395 |> 
  data.frame() |> 
  ggplot(aes(x = cost_distance, y = Dist..km.)) + 
  geom_point() + stat_smooth(method = "lm") + 
  coord_equal()
# cor = 0.57, t = 280.11, df = 160167, p-value < 2.2e-16
cor.test(ffr_1a4_poun_points_3395$cost_distance, ffr_1a4_poun_points_3395$Dist..km.)
