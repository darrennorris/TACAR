...
The rows can be easily converted for use in a stage-based matrix population model. 
The first row becomes the following stage-based matrix, where columns reflect 
the stage and rows what to the stage at time + 1.
```{r mpm-example}
...
x <- cdb_build_cdb(mat_a = l_pop_mat, metadata = meta)
# Seperate into U, F, C
mat_u1 <- rbind(
...
gen_time(matU = matU(my_comadre)[[1]], matF(my_comadre)[[1]], method = "cohort")
```
...
Viable when not declining as measured by lower quartile of lambda. 
