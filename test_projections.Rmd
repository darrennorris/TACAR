...
dout_all |> 
  filter(akey == "poun_base_0.2", modelf == "Stochastic uniform", ayear == 42) |>
  filter(ayear %in% c(ceiling(gen_3), ceiling(gen_3_q25), ceiling(gen_3_q75), 100)) |> 
  dplyr::group_by(akey, modelf, typef, first_year, ayear, arun) |> 
  dplyr::summarise(fem_t0 = mean(fem_t0), 
            fem = mean(fem),
            fem_diff = mean(fem_diff)
  ) |> ungroup() |> 
  dplyr::group_by(akey, modelf, typef, first_year, ayear) |> 
  dplyr::summarise(acount = n(), 
            fem_t0 = mean(fem_t0), 
            fem = mean(fem),
            fem_min = min(fem),
            fem_max = max(fem),
...
model_ref2 <- model_ref |> left_join(dout_all |> ungroup() |>
  filter(ayear %in% c(ceiling(gen_3), ceiling(gen_3_q25), ceiling(gen_3_q75), 100)) |> 
  ...
  summarise(fem_t0 = mean(fem_t0), 
            fem = mean(fem),
            fem_sd = sd(fem, na.rm = TRUE),
            fem_diff = mean(fem_diff), 
            fem_diff_sd = sd(fem_diff, na.rm = TRUE), 
            fem_diff_q25 = quantile(fem_diff, probs = 0.25, na.rm = TRUE),
            fem_diff_q75 = quantile(fem_diff, probs = 0.75, na.rm = TRUE)) |> 
  ungroup() |> 
