...
# load points made previously in prep_river_points.Rmd. RIVORD 1 to 4. 
...
# Now with free-flowing river points.  RIV ORD 1 to 4. 
# Cropped to basins from Norris et. al. 2019.
# Here 3 versions - modelkey pess, BAU, optimistic.
scenario_ffr_res <- ffr_1a4_poun_points_3395 |> data.frame() |> 
  #filter(subbasn == "Trombetas") |>
  mutate(Protected = if_else(All == 1, "yes", "no"), 
         Accessible = if_else(access_new == 0, "no", "yes"), 
         Free.flowing = if_else(CSI >= 0.92, "yes", "no")
         ) |> left_join( 
           actions |> 
  filter(Podocnemis.modelling.scenario == "Protection", 
         flag_ff == "CSI_FF") |> 
...
saveRDS(scenario_ffr_res, "inst/other/scenario_res_ffr.rds")
```
