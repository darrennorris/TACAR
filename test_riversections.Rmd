...
dout <- join_sf |> 
  select(BASIN_N, subbasn, All, accessible, COUNTRY, BAS_NAME, 
         RIV_ORD, BB_ID, BB_NAME, CSI, CSI_FF) |> 
  data.frame()
return(dout)
}
tout <- plyr::adply(df_points[c(1, 11, 33, 17, 46), ], 
...
scenario_res <- tout |> data.frame() |> 
  ...
  mutate(flag_b = paste(BASIN_N, subbasn, sep="_"))
...
Make colourblind friendly pallette.
```{r make-palette}
...
ggplot(aes(x = flag_50_42y, y = rl)) + 
  ...
  coord_flip() + 
  facet_grid(subbasn ~ model_namef) + 
  theme_bw() +
...
ggplot(aes(x = flag_50_42y, y = ml)) + 
  ...
  labs(y = "lambda", x = "Endangered (50% loss)") + 
  coord_flip() + 
  facet_grid(subbasn ~ model_namef) + 
  theme_bw() +
  theme(legend.position="none")
...
# Try to use bbox for sensible breaks. yaxis usually ok. xaxis squashed
# All on western hemisphere so this works
inp <- x
...
fig_map <- inp |> 
  ...
  theme_bw()
ba <- paste(x[1, 'BASIN_N'], "_", x[1, 'subbasn'], sep = "")
...
# Juruá 
selb <- which(scenario_res$subbasn == "Abacaxis")
selb <- which(scenario_res$subbasn == "Juruá")
plyr::d_ply(scenario_res, .variables = c("flag_b"),
                     .fun = make_maps)

fig_test <- readRDS("inst/other/map_t42_Amazon_Abacaxis.rds")
fig_test <- readRDS("inst/other/fig_rds/map_t42_Amazon_Juruá.rds")
```
